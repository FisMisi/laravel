...
<h2>Model Type Categories</h2>
    ...
    <a class="btn btn-primary btn-sm" href="{{ route('/administrator/model_categories/cat/{type_id}/{id}', array('type_id'=>$helperDataJson['categoryTypeId'],'id' => 0)) }}"> New subcategory</a>
    
        @if($helperDataJson['needPager'])
                <div>SubcategoryNum (to Select):{{$helperDataJson['categoriesCount']}}</div> 
                Page:
        ...
        <script>
            $("#searchbutton").click(function () 
            {
                ...
                if (ACTIVE == 2 && LIMIT == 20 && (PAGE == 1 || PAGE == undefined)) {
                        window.location = "/administrator/model_categories/"+{{$helperDataJson['categoryTypeId']}};
                } else  {
                        if (ACTIVE != 2) {
                        ...
                        window.location = "/administrator/model_categories/"+{{$helperDataJson['categoryTypeId']}}+"?"+filtersArray.join("&");
                } 
            });
        </script>
				...
				@if (isset($category))
					<tr>
						...
						<td>{{$category['title']}}</td>
                                                ...
                                                <td>{{ Form::open(array('route' => array('/administrator/model_categories/cat/update_statusz/{id}',$category['id']), 'role' => 'form')) }}
                                                    <a class="btn btn-info btn-sm" role="button" href={{route('/administrator/model_categories/cat/{type_id}/{id}', array('type_id' => $helperDataJson['categoryTypeId'],'id' => $category['id']))}} >View</a>
                                                    <button type="submit" name="/administrator/model_categories/cat/update_statusz/{id}" class="btn btn-success btn-sm">
